    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <header className="bg-white shadow-sm sticky top-0 z-20 border-b border-slate-100">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={onBack} className="p-2 -ml-2 text-slate-800 hover:bg-slate-100 rounded-full transition-colors"><ArrowLeft className="w-6 h-6" /></button>
      ...
      <div className="max-w-3xl mx-auto w-full px-4 pt-4 space-y-4">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex gap-3 items-start">
          ...
          <p className="text-xs text-yellow-800 leading-relaxed font-medium">Los casos se borrarán automáticamente en 30 días, si deseas que no se ejecute, haz click en el mismo y activa el botón de proteger.</p>
        </div>
        <div className="relative">
          <input type="text" placeholder="BUSCAR POR NOMBRE O DIAGNÓSTICO..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500/20 font-bold uppercase text-sm" /><Search className="w-5 h-5 text-slate-400 absolute left-3 top-3.5" />
        </div>
      </div>
      <main className="flex-grow w-full max-w-3xl mx-auto px-4 py-4 space-y-3">
        {filteredCases.length === 0 ? (
          <div className="text-center py-10 text-slate-400">
            ...
            <p className="font-bold">Sin casos recientes</p>
          </div>
