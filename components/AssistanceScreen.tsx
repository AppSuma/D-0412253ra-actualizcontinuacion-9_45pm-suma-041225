...
export const AssistanceScreen: React.FC<AssistanceScreenProps> = ({ onEndSession, patientData, userRole }) => {
  ...
  // FIX: Add ref to track message finality for proper streaming display.
  const lastMessageIsFinal = useRef(true);
  ...
  // FIX: Destructure setOnMessage and correct the model name.
  const { connect, disconnect, connected, isVolume, setOnMessage } = useLiveAPI({
    model: "gemini-2.5-flash-native-audio-preview-09-2025",
    systemInstruction: systemInstruction
  });

  // FIX: Replace old message handling with a more robust version that uses `endOfTurn`.
  useEffect(() => {
    setOnMessage((text, sender, endOfTurn) => {
      if (endOfTurn) {
        lastMessageIsFinal.current = true;
        return;
      }
      ...
      setMessages((prev) => {
        ...
        if (!lastMessageIsFinal.current && lastMsg && lastMsg.sender === sender) {
          const updatedMsg = { ...lastMsg, text: lastMsg.text + text };
          return [...prev.slice(0, -1), updatedMsg];
        } else {
          lastMessageIsFinal.current = false;
  ...
  const handleSendMessage = async () => {
    ...
    try {
        ...
        // FIX: Simplified the `contents` payload for a single text prompt.
        ...
        // FIX: Use the `.text` property to correctly extract the response text.
